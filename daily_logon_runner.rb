...
FileUtils.cp(WORKSPACE + '/lazy_logout.lic', LICH_DIR + '/scripts/lazy_logout.lic')
...
char_strings.each { |cs|
	...
	system("ruby #{LICH_DIR}/lich.rb --login #{cs[0]} --gemstone --without-frontend --detachable-client=8787 --start-scripts=lazy_logout")
}
